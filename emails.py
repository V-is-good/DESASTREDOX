...
def main():
    email = input(f'\n{Colores.azul}[⏈⃟𝒦⃟⚖] Ingresa el email: ')
    if email == "" or email == " ":
    ...
    print(f'\n{Colores.azul}[⏈⃟𝒦⃟⚖] Buscando email en archivos pdf...')
    time.sleep(3)
    for i in search(command2, tld, num=10, stop=10, pause=2):
        print(f'\nResultados encontrados!: {i}')
    print(f'\n{Colores.dorado}[⏈⃟𝒦⃟⚖] Buscando correo electronico en redes sociales...')
    ...
    print(f'\n{Colores.verde}[⏈⃟𝒦⃟⚖] Iniciando socialscan...')
    time.sleep(3)
    print('')
    os.system(f"socialscan {email}")
    print(f'\n{Colores.azul}[⏈⃟𝒦⃟⚖] Escaneo completo.')
